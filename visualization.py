...
def visualize_result(frame, result, confidence):
    """ Visualizes the predicted boxes made by the model on the input frame. """
    for box in result.boxes:
        ...
        if box_conf >= confidence:
            ...
            cv2.rectangle(frame, (x_min, y_min), (x_max, y_max), (255, 204, 229), 5)  # Draw the bounding box in bright purple :)
...
def visualize_predictions_on_video(model, video_path, save_file, w, h, confidence):
    """ Visualizes the predictions made by the model on the input video. """
